...
test_util_monotonic_time_add_msec(void *arg)
{
  ...
  /* The _msec32 variant quantizes with a fixed >>20 shift to avoid a
   * 64-bit division. Its wall-clock quantum is 2^20 abstime ticks,
   * which is ~1ms where one tick is a nanosecond, but ~44ms on Apple
   * silicon where the mach timebase is 125/3 -- up to ~3% on these
   * sub-second intervals. Widen the bound on that platform only; every
   * other platform keeps the tight check. */
