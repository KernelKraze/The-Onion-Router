...
crypto_global_init(int useAccel, const char *accelName, const char *accelDir)
{
  if (!crypto_global_initialized_) {
    ...
    /* OpenSSL 3.x providers are initialized lazily on first use.
     * Doing that from a worker thread races with other threads and
     * can crash.  Force the RSA provider to initialize now, before
     * the thread pool is created. */
    {
